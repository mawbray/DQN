...
disc1   = np.array([0.5])                    # discount factor in back-allocation
controls = np.array([0, 3, 6], dtype = np.float64)

# Experiment defintions: env, agent, controls, episodes
epochs              = 1000                                      # number of training epochs
episodes            = 2000                                      # number of episodes per epoch                                                                          
UB                  = [6, 0]                                    # upper bound on mean
...
agent_dict          = { 'obs_size': 2,
                        ...
                        'hs_2': int(18),
                        'output_size': int(3),
                        'n_holds': 200}                        # constructing agent and indicating training
        


#run training 
env                         = Model_env(p, steps_, tf, x0, **env_dict)                          # calling environment
agent                       = QLAgent(steps_, controls, **agent_dict)                                     # calling agent
...
np.save("C:\\Users\\g41606mm\\Dropbox\\Projects\\RL\\DQN\\DQNMod\\Results\\reward_training.py" , reward_training)
np.save("C:\\Users\\g41606mm\\Dropbox\\Projects\\RL\\DQN\\DQNMod\\Results\\reward_validation_mean.py" , rv_m)
