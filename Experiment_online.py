...
class Experiment(object):
    def __init__(self, env, agent, episodes, epochs, disc1,**kwargs):
      self.env , self.agent     = env, agent 
      self.episodes             = episodes
      self.disc1                = disc1
      self.epochs               = epochs
      self.updates              = kwargs.get('updates_per_epoch', 1000)      # number of weight updates per epoch
      ...
      for jj in tqdm(range(epochs)):
          ...
          for ei in (range(episodes)):
            ...
            for s in range(movements):
                ...
                prev_state          = current_state
                action              = self.agent.act(current_state, jj)                   # select control for this step from that possible
                ...
                self.agent.Observe(prev_state, action, rwd, current_state, done)

            terminal_state      = current_state
            obj_fun             = self.objective(terminal_state)
            reward.append(obj_fun)

            x   = np.round(ei/episodes,3)
            y   = list(np.round(np.linspace(0,1,self.updates),3))
          ...
          self.agent.StoreEpisode(xt, tt, c_hist, rwrd, jj, reward)
          ...
          if jj % (evaluation_period * epochs) == 0:
              ...
              for x in range(valid_eps):
                  ...
                  for s in range(movements):
                        ...
                        action          = self.agent.act(current_state, jj ,1)                 # select control for this step from that possible
                        ode             = scp.ode(self.env.model)                       # define ode
                        ode.set_integrator('lsoda', nsteps=3000)                        # define integrator
                        ode.set_initial_value(current_state,dt)                         # set initial value
                        ode.set_f_params(action)                                        # set control action
